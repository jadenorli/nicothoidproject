...
```{r}
#plot the number of adults over time per 1000 host eggs
timeseries_adult <- ggplot(data = adult_all_avg_new_day, 
...
### ii) Juveniles
```{r}
#scale up the number of each life stage from the egg mortality data 
...
timeseries_juveniles <- ggplot(data = scaled_data_egg_mort, 
       aes(x = day, y = scaled_juveniles)) +
  geom_point(color = juveniles) +
  theme_gray() +
  ylab("Larval Nicothoids per One Million Crab Eggs") +
  xlab("Development Day") + 
  #ggtitle(label = "Larval Nicos") +
  theme(plot.title = element_text(hjust = 0.5),
        axis.title.y = element_text(size = 10),
        axis.title.x = element_text(size = 10))
...
timeseries_total_nicos <- ggplot(data = scaled_data_egg_mort, 
...
timeseries_nico_eggs <- ggplot(data = nico_egg_all_avg_new_day, 
