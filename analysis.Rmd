...
autoinfection <- read_csv("NicothoidProjectAnalysis/RawData/first_broods_only.csv") %>% 
  ...
  mutate(color = case_when(eds %in% c("A", "B", "C") ~ "gold1",
                           eds %in% c("D", "E", "F") ~ "darkgoldenrod1",
                           eds %in% c("G", "H", "I") ~ "coral2",
                           eds %in% c("J", "K", "L", "M") ~ "brown4"))#read in the autoinfection data for all of the broods
...
metadata <- read_csv("NicothoidProjectAnalysis/RawData/metadata.csv") %>%
  clean_names()

#read in the distribution data
distribution <- read_csv("NicothoidProjectAnalysis/RawData/choniosphaera_prevelance.csv") %>% 
  clean_names() 
...
adult_counts <- autoinfection %>% 
  dplyr::select(experiment_id, crab_id, date, eds, number_of_adult_copepods, number_of_nemerteans, total_number_of_eggs, number_of_copepod_eggs) %>% 
  ...
  dplyr::select(experiment_id, crab_id, date, eds, number_adult_nicos, number_of_nemerteans, total_number_crab_eggs, number_nico_eggs) %>% 
...
adult_counts_all_broods <- all_broods %>% 
  dplyr::select(experiment_id, crab_id, date, egg_development_stage, number_of_adult_copepods, number_of_nemerteans, total_number_of_eggs, number_of_copepod_eggs) %>% 
...
#create a dataframe with the average number of adults 
...
##autoinfection data from first broods and egg mortality
#create a dataframe with the proportion of 
...
##autoinfection data from all broods and egg mortality
#create a dataframe with the proportion of nico eggs 
...
# want to make a summary table now with extrapolated count assuming 1 million eggs, total count of nicos, total number of crab eggs searched, number of observations on that crab, all per crab    
adult_counts_summary <- adult_counts_new %>% 
  group_by(crab_id) %>% 
  summarise(total_nicos = sum(adult_nicos),
            mean_nicos_per_count = mean(adult_nicos),
            observations = n(), 
...
adult_summary_new <- adult_counts_new %>%
  mutate(prop_adult_nicos = adult_nicos/crab_eggs) %>%
  mutate(scaled_prop_adult_nicos = prop_adult_nicos*1000000) %>%
  mutate(prop_adult_nicos_sample = prop_adult_nicos*1000) %>%
  group_by(crab_id) %>% 
  summarise(total_nicos = sum(adult_nicos),
            mean_nicos_per_count = mean(adult_nicos),
            observations = n(), 
            eggs_searched = sum(crab_eggs),
            adults_scaled_sum = sum(scaled_prop_adult_nicos),
            adults_sample_sum = sum(prop_adult_nicos_sample)) %>%
  mutate(adults_per_egg = adults_sample_sum/eggs_searched) %>% 
  mutate(adults_per_egg_sample = adults_per_egg*1000) %>% 
  mutate(adults_extrapolated_1mil = adults_per_egg*1000000) %>%
  mutate(nicos_per_egg = total_nicos/eggs_searched) %>% 
  mutate(nicos_per_egg_sample = nicos_per_egg*1000) %>% 
  mutate(nicos_extrapolated_1mil = nicos_per_egg*1000000) %>%
  as_tibble()
  
#subset the adult_summary_new into low and high infestations
high_summary <- adult_summary_new %>%
  filter(crab_id %in% c("ANT-GAV-034", "ANT-GAV-037", "ANT-GAV-045", "ANT-GAV-051", "ANT-GAV-052", "ANT-GAV-053", "ANT-GAV-060"))
  
low_summary <- adult_summary_new %>%
...
#make groups with the days to condense the points more 
adult_all_group <- adult_all_new %>%
  mutate(group = case_when(day %in% c(1:5) ~ "1",
                           day %in% c(6:10) ~ "2",
                           day %in% c(11:15) ~ "3",
                           day %in% c(16:20) ~ "4",
                           day %in% c(21:25) ~ "5",
...
nico_egg_all_avg_new_day <-  nico_egg_all_new %>%
  ...
  mutate(min_scaled_nico_eggs = min_prop_nico_eggs*1000000) %>%
  mutate(sd_scaled_nico_eggs = sd_prop_nico_eggs*1000000) %>% 
  mutate_at(c(1:9), ~replace(., is.na(.), 0)) %>%
  mutate(max_sd = (scaled_nico_eggs + sd_scaled_nico_eggs)) %>%
  mutate(min_sd = (scaled_nico_eggs - sd_scaled_nico_eggs)) %>%
  mutate_at("min_sd", ~replace(., min_sd < 0, 0))%>%
  as_tibble()
...
#make groups with the days to condense the points more 
nico_egg_all_group <- nico_egg_all_new %>%
  mutate(group = case_when(day %in% c(1:5) ~ "1",
                           day %in% c(6:10) ~ "2",
                           day %in% c(11:15) ~ "3",
                           day %in% c(16:20) ~ "4",
...
##### - egg mortality prop dataframe
```{r}
#create a dataframe with the proportions of adults in a sample and scaled
egg_mort <- egg_mortality_day %>%
  mutate(prop_adult_nicos = number_adult_nicos/number_viable_crab_eggs) %>%
  mutate(scaled_prop_adult_nicos = prop_adult_nicos*1000000) %>%
  mutate(prop_adult_nicos_sample = prop_adult_nicos*1000) 


high_crabs <- high_crab_id %>%
  mutate(prop_adult_nicos = number_adult_nicos/number_viable_crab_eggs) %>%
  mutate(scaled_prop_adult_nicos = prop_adult_nicos*1000000) %>%
  mutate(prop_adult_nicos_sample = prop_adult_nicos*1000) 


low_crabs <- low_crab_id %>%
  mutate(prop_adult_nicos = number_adult_nicos/number_viable_crab_eggs) %>%
  mutate(scaled_prop_adult_nicos = prop_adult_nicos*1000000) %>%
  mutate(prop_adult_nicos_sample = prop_adult_nicos*1000) 
...
#select the low infested crabs only from the timeseries dataframe 
...
crab_id <- egg_mortality_day %>%
  ...
  mutate(prop_adult_nicos_sample = prop_adult_nicos*1000) %>%
  mutate(prop_juvenile_nicos_sample = prop_juvenile_nicos*1000) %>%
  mutate(prop_total_nicos_sample = prop_total_nicos*1000) %>%
  mutate(prop_nico_eggs_sample = prop_nico_eggs*1000) %>%
  group_by(crab_id) %>%
    summarise(adults = sum(number_adult_nicos),
              ...
              adults_sample = sum(prop_adult_nicos_sample),
              juveniles_sample = sum(prop_juvenile_nicos_sample),
              total_nicos_sample = sum(prop_total_nicos_sample),
              nico_eggs_sample = sum(prop_nico_eggs_sample)) %>%
  as_tibble()
...
totals_per_crab_id <- egg_mortality_day %>% 
  ...
  mutate(adults_per_1000 = adults_per_egg*1000) %>%
  as_tibble()
...
clean_metadata <- metadata %>%
  filter(!grepl("exclude", notes_15)) %>%
  filter(!is.na(carapace_width)) %>%
  dplyr::select(!c(10,15))

#look at only the Gaviota Coast crabs 
gaviota <- clean_metadata %>%
  filter(location == "Gaviota Coast")

#look at all females examined 
female_metadata <- clean_metadata %>%
  filter(sex == "F") 

#look at only initially gravid females
gravid_metadata <- female_metadata %>%
  filter(gravid == "Y")

#look at all males examined 
...
eggs_mort_vis_low <- ggplot(data = eggs_mort_day_avg_low, 
       aes(x = day, y = (avg_prop_viable_eggs*100))) +
  geom_point(color = "red3") + 
  theme_gray() +
  ylab("Average Percent Viable Crab Eggs") +
  xlab("Development Day") + 
  ggtitle(label = "Average Egg Mortality Across Development Day",
          subtitle = "Low Infestation Crabs") +
  theme(plot.title = element_text(hjust = 0.5),
...
eggs_mort_vis_high <- ggplot(data = eggs_mort_day_avg_high, 
  ...
  geom_point(color = "red3") +
  theme_gray() +
  ylab("Average Precent Viable Crab Eggs") +
  xlab("Development Day") + 
  ggtitle(label = "Average Egg Mortality Across Development Day",
          subtitle = "High Infestation Crabs") +
  theme(plot.title = element_text(hjust = 0.5),
...
## e) Publication Timeseries Trials
...
adult_timeseries_all_new_day <- ggplot() +
  ...
  #geom_line(data = adult_all_new,
            #aes(x = day, y = scaled_adults, group = crab_id), color = adults) +
  theme_classic() +
  ylab("Adult Nicothoids per One Million Crab Eggs") +
  xlab("Development Day") + 
  #ggtitle(label = "Adult Nicos") +
...
eds_adults <- ggplot() +
  ...
  #geom_ribbon(data = adult_all_avg_new,
              ...
              #alpha = 0.4) +
  geom_point(data = adult_all_avg_new,
            aes(x = eds, y = scaled_adults), color = adults) +
  theme_gray() +
...
eds_nico_eggs <- ggplot() +
  ...
  #geom_ribbon(data = nico_egg_all_avg_new,
              ...
              #alpha = 0.4) +
  geom_point(data = nico_egg_all_avg_new,
            aes(x = eds, y = scaled_nico_eggs), color = adults) +
  theme_gray() +
...
```{r}
#plot the number of adults over time per 1000 host eggs
timeseries_adult_final_sd <- ggplot(data = adult_all_avg_new_day, 
            aes(x = day, y = scaled_adults)) +
  geom_line(data = adult_all_avg_new_day,
            aes(x = day, y = scaled_adults), color = adults) +
  geom_point(data = adult_all_avg_new_day,
            aes(x = day, y = scaled_adults), color = adults) +
  geom_errorbar(aes(ymin = (scaled_adults-sd_scaled_adults), 
                      ymax = (scaled_adults+sd_scaled_adults)),
                width = 0.4) +
  theme_gray() +
  ylab("Adult Nicothoids per One Million Crab Eggs") +
  xlab("Development Day") + 
...
```{r}
#plot the number of adults over time per 1000 host eggs
timeseries_adult_final_mm <- ggplot(data = adult_all_avg_new_day, 
            aes(x = day, y = scaled_adults)) +
  geom_line(data = adult_all_avg_new_day,
            aes(x = day, y = scaled_adults), color = adults) +
  geom_point(data = adult_all_avg_new_day,
            aes(x = day, y = scaled_adults), color = adults) +
  geom_errorbar(aes(ymin = (min_scaled_adults), 
                      ymax = (max_scaled_adults)),
                width = 0.4) +
  theme_gray() +
  ylab("Adult Nicothoids per One Million Crab Eggs") +
...
```{r}
#plot the number of adults over time per 1000 host eggs
timeseries_adult_final_sd_test <- ggplot() +
  geom_line(data = adult_all_group,
            aes(x = avg_day, y = scaled_adults), color = adults) +
  geom_errorbar(data = adult_all_group,
                aes(x = avg_day, y = scaled_adults,
                    ymin = min_sd, 
                    ymax = max_sd),
                width = 0.4) +
  theme_gray() +
  scale_y_continuous(limits = c(0.95 * min(adult_all_group$min_sd), 
                                1.05 * max(adult_all_group$max_sd))) +
  ylab("Adult Nicothoids per One Million Crab Eggs") +
  xlab("Development Day") + 
  theme(plot.title = element_text(hjust = 0.5),
        axis.title.y = element_text(size = 10),
...
timeseries_adult_final_sd_test
...
```{r}
#plot the number of adults over time per 1000 host eggs
timeseries_juvenile_final_sd <- ggplot(data = timeseries_avg,
            aes(x = day, y = scaled_juveniles)) +
  geom_line(data = timeseries_avg,
            aes(x = day, y = scaled_juveniles), color = adults) +
  geom_point(data = timeseries_avg,
            aes(x = day, y = scaled_juveniles), color = adults) +
  geom_errorbar(aes(ymin = (scaled_juveniles-sd_scaled_juveniles), 
                      ymax = (scaled_juveniles+sd_scaled_juveniles)),
                width = 0.4) +
  theme_gray() +
  ylab("Larval Nicothoids per One Million Crab Eggs") +
  xlab("Development Day") + 
...
```{r}
#plot the number of adults over time per 1000 host eggs
timeseries_juvenile_final_mm <- ggplot(data = timeseries_avg,
            aes(x = day, y = scaled_juveniles)) +
  geom_line(data = timeseries_avg,
            aes(x = day, y = scaled_juveniles), color = adults) +
  geom_point(data = timeseries_avg,
            aes(x = day, y = scaled_juveniles), color = adults) +
  geom_errorbar(aes(ymin = (min_scaled_juveniles), 
                      ymax = (max_scaled_juveniles)),
                width = 0.4) +
  theme_gray() +
  ylab("Larval Nicothoids per One Million Crab Eggs") +
...
```{r}
#plot the number of adults over time per 1000 host eggs
timeseries_nico_eggs_final_sd <- ggplot(data = nico_egg_all_avg_new_day,
            aes(x = day, y = scaled_nico_eggs)) +
  geom_line(data = nico_egg_all_avg_new_day,
            aes(x = day, y = scaled_nico_eggs), color = adults) +
  geom_point(data = nico_egg_all_avg_new_day,
            aes(x = day, y = scaled_nico_eggs), color = adults) +
  geom_errorbar(aes(ymin = (scaled_nico_eggs-sd_scaled_nico_eggs), 
                      ymax = (scaled_nico_eggs+sd_scaled_nico_eggs)),
                width = 0.4) +
  theme_gray() +
  ylab("Nicothoid Eggs per One Million Crab Eggs") +
  xlab("Development Day") + 
...
```{r}
#plot the number of adults over time per 1000 host eggs
timeseries_nico_eggs_final_mm <- ggplot(data = nico_egg_all_avg_new_day, 
            aes(x = day, y = scaled_nico_eggs)) +
  geom_line(data = nico_egg_all_avg_new_day,
            aes(x = day, y = scaled_nico_eggs), color = adults) +
  geom_point(data = nico_egg_all_avg_new_day,
            aes(x = day, y = scaled_nico_eggs), color = adults) +
  geom_errorbar(aes(ymin = (min_scaled_nico_eggs), 
                      ymax = (max_scaled_nico_eggs)),
                width = 0.4) +
  theme_gray() +
  ylab("Nicothoid Eggs per One Million Crab Eggs") +
...
```{r}
#plot the number of adults over time per 1000 host eggs
timeseries_nico_eggs_final_sd_test <- ggplot() +
  geom_line(data = nico_egg_all_group,
            aes(x = avg_day, y = scaled_nico_eggs), color = adults) +
  geom_errorbar(data = nico_egg_all_group,
                inherit.aes = FALSE,
                aes(x = avg_day, y = scaled_nico_eggs,
                    ymin = min_sd, 
                    ymax = max_sd),
                width = 0.4) +
  theme_gray() +
  #scale_x_continuous(sec.axis = sec_axis(~e)) +
  ylab("Nicothoid Eggs per One Million Crab Eggs") +
  xlab("Development Day") + 
  theme(plot.title = element_text(hjust = 0.5),
        axis.title.y = element_text(size = 10),
...
timeseries_nico_eggs_final_sd_test

```


# 3. Adult Counts Visualization


## - random visualizations 
```{r}
#plot the number of adult nicos against the total number of viable eggs
adult_plot_test_low <- ggplot(data = egg_mort, 
       aes(y = prop_viable_eggs, x = prop_adult_nicos)) +
  geom_point(data = egg_mort, 
             aes(color = color)) + 
...
#plot the number of adult nicos against the total number of viable eggs
adult_plot_test_high <- ggplot(data = egg_mort, 
       aes(y = prop_viable_eggs, x = prop_adult_nicos_sample)) +
  geom_point(data = egg_mort, 
             aes(color = color)) + 
  scale_color_manual(labels = c("D-F", "G-I", "J-L"),
                     values = c("darkgoldenrod1" = "darkgoldenrod1",
...
adult_plot_test_high


#plot the number of adult nicos against the total number of viable eggs
low_adult_test <- ggplot(data = low_crabs, 
       aes(y = prop_viable_eggs, x = prop_adult_nicos_sample)) +
  geom_point(data = low_crabs, 
             aes(color = color)) + 
  scale_color_manual(labels = c("D-F", "G-I", "J-L"),
                     values = c("darkgoldenrod1" = "darkgoldenrod1",
                               "coral2" = "coral2",
...
#plot the number of adult nicos against the total number of viable eggs
high_adult_test <- ggplot(data = high_crabs, 
       aes(y = prop_viable_eggs, x = prop_adult_nicos_sample)) +
  geom_point(data = high_crabs, 
             aes(color = color)) + 
  scale_color_manual(labels = c("D-F", "G-I", "J-L"),
                     values = c("darkgoldenrod1" = "darkgoldenrod1",
                               "coral2" = "coral2",
...
## - two plots
```{r}
#plot the prop of viable eggs over time overlayed with the prop of adult nicos over time 
```
...
```{r}
#plot the number of adult nicos against the total number of viable eggs
adult_plot_esa <- ggplot(data = egg_mortality, 
       aes(y = (prop_viable_eggs*100), x = number_adult_nicos)) +
  geom_point(data = egg_mortality, color = adults) +
  geom_smooth(method = lm, level = 0.95, color = "black") +
  theme_gray() +
  xlab("Number of Adult Nicothoids") +
  ylab("Percent of Viable Crab Eggs") + 
  ggtitle("Adult Nicothoids") +
  theme(plot.title = element_text(hjust = 0.5),
        axis.title.y = element_text(size = 6),
        axis.title.x = element_text(size = 8),
        legend.position = "none") + 
...
juvenile_plot_esa <- ggplot(data = egg_mortality, 
       aes(y = (prop_viable_eggs*100), x = number_juvenile_nicos)) +
  geom_point(data = egg_mortality, color = juveniles) +
  geom_smooth(method = lm, level = 0.95, color = "black") +
  theme_gray() +
  xlab("Number of Larval Nicothoids") +
  ylab("Percent of Viable Crab Eggs") + 
  ggtitle("Larval Nicothoids") +
  theme(plot.title = element_text(hjust = 0.5),
        axis.title.y = element_text(size = 6),
        axis.title.x = element_text(size = 8),
        legend.position = "none") + 
...
total_plot_esa <- ggplot(data = egg_mortality, 
       aes( y = (prop_viable_eggs*100), x = total_nicos)) +
  geom_point(data = egg_mortality, color = totals) +
  geom_smooth(method = lm, level = 0.95, color = "black") +
  theme_gray() +
  xlab("Total Number of Nicothoids (Larvae/Adults)") +
  ylab("Percent of Viable Crab Eggs") + 
  ggtitle("Total Nicothoids") +
  theme(plot.title = element_text(hjust = 0.5),
        axis.title.y = element_text(size = 6),
        axis.title.x = element_text(size = 8),
        legend.position = "none") + 
...
nico_egg_plot_esa <- ggplot(data = egg_mortality, 
       aes(y = (prop_viable_eggs*100), x = number_nico_eggs)) +
  geom_point(data = egg_mortality, color = "orangered4") + 
  geom_smooth(method = lm, level = 0.95, color = "black") +
  theme_gray() +
  xlab("Number of Nicothoid Eggs") +
  ylab("Percent of Viable Crab Eggs") + 
  ggtitle("Nicothoid Eggs") +
  theme(plot.title = element_text(hjust = 0.5),
        axis.title.y = element_text(size = 6),
        axis.title.x = element_text(size = 8),
        legend.position = "none") + 
...
#plot the number of nico eggs against the total number of viable eggs
nico_egg_plot_esa_2 <- ggplot(data = egg_mortality, 
       aes(y = (prop_viable_eggs*100), x = number_nico_eggs)) +
  geom_point(data = egg_mortality, color = "orangered4") + 
  geom_smooth(method = lm, level = 0.95, color = "black") +
  theme_gray() +
...
#add the plots together
esa <- ggarrange(adult_plot_esa, juvenile_plot_esa, total_plot_esa, nico_egg_plot_esa,
                           labels = c("a)", "b)", "c)", "d)"),
                           ncol = 2, nrow = 2)
esa
...
freqdist_eggs_adults_crab_id <- gghistogram(data = crab_id, 
  ...
  xlab("Mean Count of Adult Nicothoids per One Million Crab Eggs") + 
  #ggtitle("Adult Nicos") +
  theme(plot.title = element_text(hjust = 0.5)) +
...
#use Freedman Diaconis rule to determine optimal binwidth:
bw_eggs_adults_esa<- 2*IQR(crab_id$adults_per_1000)/length(crab_id$adults_per_1000)^(1/3)

#create a histogram of the distribution of adults (averaged per totals)
freqdist_eggs_adults_esa <- gghistogram(data = totals_per_crab_id, 
                              x = "adults_per_1000", 
                              fill = adults,
                              binwidth = bw_eggs_adults_esa)+
  ylab("Frequency of Crabs") +
  xlab("Average Count of Adult Nicothoids per One Thousand Crab Eggs") + 
  #ggtitle("Adult Nicos") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_y_continuous(expand = c(0,0)) +
  #geom_vline(aes(xintercept = 20),color = "red", linewidth = 1)+
...
```{r a nice freq dist histogram}
#use Freedman Diaconis rule to determine optimal binwidth:
bw_new <- 2*IQR(adult_summary_new$nicos_per_egg_sample)/length(adult_summary_new$nicos_per_egg_sample)^(1/3)

bw_new_2 <- 2*IQR(adult_summary_new$nicos_extrapolated_1mil)/length(adult_summary_new$nicos_extrapolated_1mil)^(1/3)
...
freqdist_adults_new <- gghistogram(data = adult_summary_new, 
                              x = "nicos_per_egg_sample", 
                              fill = "red4",
                              binwidth = bw_new)+
  ylab("Frequency of Crabs") +
  xlab("Mean Count of Adult Nicothoids per One Thousand Crab Eggs") + 
  #ggtitle("Adult Nicos") +
  theme(plot.title = element_text(hjust = 0.5))+
  scale_y_continuous(expand = c(0,0), 
                     limits = c(0,12)) +
  xlim(0, 40) +
  geom_vline(aes(xintercept = 30),color = "red", linewidth = 1)+
  theme_classic()
...
### c) Histogram for Juveniles 

#### i) New Autoinfection Data
```{r}
#use Freedman Diaconis rule to determine optimal binwidth:
bw_new_juv <-2 *IQR(adult_summary_new$nicos_per_egg_sample)/length(adult_summary_new$nicos_per_egg_sample)^(1/3)
...
model <- timeseries %>%
  dplyr::select(c(2,4,8,12:14,17,19,24))
...
gill_density <- dissections %>%
  mutate(scaled_total_nicos = nicothoid_count*12)
...
clean_dissections <- gill_density %>%
  filter(!dissection_id %in% c("Dissection-027", "Dissection-028", "Dissection-047"))

##sample size n = 35

#calculate the mean density of the nicos in the gills
mean_density_gills <- mean(clean_dissections$scaled_total_nicos)
mean_density_gills

#calculate standard deviation of mean density of the adults in the gills
mean_density_gills_sd <- sd(clean_dissections$scaled_total_nicos)
...
yellow_all_mean_density <- mean(yellow_all$scaled_total_nicos)
yellow_all_mean_density

#calculate standard deviation of the mean density
yellow_all_mean_density_gills_sd <- sd(yellow_all$scaled_total_nicos)
...
red_all_mean_density <- mean(red_all$scaled_total_nicos)
red_all_mean_density

#calculate standard deviation of the mean density
red_all_mean_density_gills_sd <- sd(red_all$scaled_total_nicos)
...
brown_all_mean_density <- mean(brown_all$scaled_total_nicos)
brown_all_mean_density

#calculate standard deviation of the mean density
brown_all_mean_density_gills_sd <- sd(brown_all$scaled_total_nicos)
...
males_all_mean_density <- mean(males_all$scaled_total_nicos)
males_all_mean_density

#calculate standard deviation of the mean density
males_all_mean_density_gills_sd <- sd(males_all$scaled_total_nicos)
...
females_all_mean_density <- mean(females_all$scaled_total_nicos)
females_all_mean_density

#calculate standard deviation of the mean density
females_all_mean_density_gills_sd <- sd(females_all$scaled_total_nicos)
...
yellow_males_mean_density <- mean(yellow_males$scaled_total_nicos)
yellow_males_mean_density

#calculate standard deviation of the mean density
yellow_males_mean_density_gills_sd <- sd(yellow_males$scaled_total_nicos)
...
red_males_mean_density <- mean(red_males$scaled_total_nicos)
red_males_mean_density

#calculate standard deviation of the mean density
red_males_mean_density_gills_sd <- sd(red_males$scaled_total_nicos)
...
#calculate the mean density of the nicos in the gills of red males
brown_males_mean_density <- mean(brown_males$scaled_total_nicos)
brown_males_mean_density

#calculate standard deviation of the mean density
brown_males_mean_density_gills_sd <- sd(brown_males$scaled_total_nicos)
...
yellow_females_mean_density <- mean(yellow_females$scaled_total_nicos)
yellow_females_mean_density

#calculate standard deviation of the mean density
yellow_females_mean_density_gills_sd <- sd(yellow_females$scaled_total_nicos)
...
red_females_mean_density <- mean(red_females$scaled_total_nicos)
red_females_mean_density

#calculate standard deviation of the mean density
red_females_mean_density_gills_sd <- sd(red_females$scaled_total_nicos)
...
#calculate the mean density of the nicos in the gills of red females
brown_females_mean_density <- mean(brown_females$scaled_total_nicos)
brown_females_mean_density

#calculate standard deviation of the mean density
brown_females_mean_density_gills_sd <- sd(brown_females$scaled_total_nicos)
...
```{r}
#calculate the mean intensity for the nicos in the gills 
male_mean_intensity_gills <- mean(males_all$intensity_nicos_gills)
...
```{r}
#calculate the mean intensity for the nicos in the gills 
female_mean_intensity_gills <- mean(females_all$intensity_nicos_gills)
...
### i) Total Nicos
```{r}
#calculate the minimum, maximum, mean, and sd from the egg mortality data alone
##total nicos
min_total_nicos <- min(crab_id$total_nicos_sample)
min_total_nicos

max_total_nicos <- max(crab_id$total_nicos_sample)
max_total_nicos

mean_total_nicos <- mean(crab_id$total_nicos_sample)
mean_total_nicos

sd_total_nicos <- sd(crab_id$total_nicos_sample)
sd_total_nicos
...
#calculate the minimum, maximum, mean, and sd from the egg mortality data alone
##juveniles
min_juveniles <- min(crab_id$juveniles_sample)
min_juveniles

max_juveniles <- max(crab_id$juveniles_sample)
max_juveniles

mean_juveniles <- mean(crab_id$juveniles_sample)
mean_juveniles

sd_juveniles <- sd(crab_id$juveniles_sample)
sd_juveniles
...
#calculate the minimum, maximum, mean, and sd from the egg mortality data alone
##nico eggs
min_nico_eggs <- min(crab_id$nico_eggs_sample)
min_nico_eggs

max_nico_eggs <- max(crab_id$nico_eggs_sample)
max_nico_eggs

mean_nico_eggs <- mean(crab_id$nico_eggs_sample)
mean_nico_eggs

sd_nico_eggs <- sd(crab_id$nico_eggs_sample)
sd_nico_eggs
...
### iv) Adults
```{r}
#calculate the minimum, maximum, mean, and sd from the combined data
##adults
min_adults <- min(adult_summary_new$nicos_per_egg_sample)
min_adults

max_adults <- max(adult_summary_new$nicos_per_egg_sample)
max_adults

mean_adults <- mean(adult_summary_new$nicos_per_egg_sample)
mean_adults

sd_adults <- sd(adult_summary_new$nicos_per_egg_sample)
sd_adults
...
juvenile_timeseries_all_counts <- ggplot() +
  ...
  scale_x_continuous(name = "Development Day",
                     breaks = seq(0, max(timeseries$day), by = 2)) +
  scale_y_continuous(labels = function(y) format(y, scientific = TRUE)) + 
  #ggtitle(label = "Adult Nicos") +
  theme(text = element_text(family = "Times New Roman"),
        plot.title = element_text(hjust = 0.5, face = "bold"),
...
```{r}
#plot the number of nico eggs over time per one million host eggs
nico_egg_timeseries_all_new_day <- ggplot() +
  geom_point(data = nico_egg_all_new,
  ...
  scale_x_continuous(name = "Development Day",
                     breaks = seq(0, max(nico_egg_all_new$day), by = 2), 0) +
  scale_y_continuous(labels = function(y) format(y, scientific = TRUE)) +
  #ggtitle(label = "Nico Eggs") +
  theme(text = element_text(family = "Times New Roman"),
        plot.title = element_text(hjust = 0.5, face = "bold"),
...
```{r}
#add the plots together
publication_timeseries <- ggarrange(total_nicos_timeseries_all_counts, juvenile_timeseries_all_counts, adult_timeseries_all_new_day, nico_egg_timeseries_all_new_day,
                           labels = c("a)", "b)", "c)", "d)"),
...
#add the plot together to add the label
freqdist_adults_new_2 <- ggarrange(freqdist_adults_new,
                           labels = "a)",
...
#save the output as as png
ggsave("adult_nico_freq_distribution_new.png", freqdist_adults_new_2, device = "png")

freqdist_adults_new_2

```

### ii) Juveniles 
```{r}
#create a histogram of the distribution of juveniles from the crabs that had data with juveniles
freqdist_juveniles_new <- gghistogram(data = totals_per_crab_id, 
                              x = "juveniles_scaled", 
                              fill = publication,
                              binwidth = bw_eggs_juveniles) +
  ylab("Frequency of Crabs") +
  xlab("Mean Count of Larval Nicothoids per One Million Crab Eggs") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_y_continuous(expand = c(0,0), 
                     limits = c(0,15)) +
  theme(text = element_text(family = "Times New Roman"),
...
#add the plot together to add the label
freqdist_juveniles_new_2 <- ggarrange(freqdist_juveniles_new,
                           labels = "b)",
                           font.label = list(size = 12, 
                                             color = "black",
                                             face = "bold",
...
#save the output as as png
ggsave("freqdist_juveniles_new.png", freqdist_juveniles_new_2, device = "png")

freqdist_juveniles_new_2
...
### iii) Total Nicos
```{r}
#create a histogram of the distribution of total nicos
freqdist_eggs_total_nicos <- gghistogram(data = totals_per_crab_id, 
                              x = "total_nicos_scaled", 
                              fill = publication,
                              binwidth = bw_eggs_total_nicos) +
  ylab("Frequency of Crabs") +
  xlab("Mean Count of Total Nicothoids per One Million Crab Eggs") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_y_continuous(expand = c(0,0)) +
  theme(text = element_text(family = "Times New Roman"),
        axis.title.x = element_text(colour = "black", face = "bold"),
...
#save the output as as png
ggsave("freqdist_eggs_total_nicos.png", freqdist_eggs_total_nicos, device = "png")
...
# 12. Map of Choniosphaera 

## World Distribution Map
```{r}
#create a dataframe with the coordinates of the world map
world <- map_data("world") %>%
  subset(region != "Antarctica")

#choose which countries to keep in the dataframe
countries <- world %>%
  subset(region %in% c("Canada", "USA", "Australia", "Vietnam", "France", "Germany", "India"))

#map the distribution of choniosphaera species
distribution_map <- ggplot() +
  geom_map(data = world, map = world, 
                    aes(y = lat, x = long, map_id = region), fill = "white", color = "black") + 
  geom_map(data = countries, map = countries, 
                    aes(y = lat, x = long, map_id = region), fill = publication_2, color = "black") +
  geom_point(data = distribution,
             shape = 20, size = 1, stroke = 1.5,
             aes(y = latitude, x = longitude, color = nico_species)) + 
  ylab("Latitude") + 
  xlab("Longitude") +
  theme(text = element_text(family = "Times New Roman"),
        axis.title.x = element_text(colour = "black", face = "bold"),
  ...
  labs(col = "Choniosphaera Species") +
  scale_color_manual(values = c("chocolate4", "goldenrod2", "brown3", "darkorange1"))

distribution_map

#save the output as as png
ggsave("distribution_map.png", distribution_map, device = "png")
