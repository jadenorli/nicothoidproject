...
# 12. Viable Eggs vs. Life Stages

## a) Viable Eggs vs. Adults
```{r}
#plot the data for the dataframe with the averages as groups
adults_viable_eggs <- ggplot(data = timeseries, 
       aes(x = scaled_viable_eggs, y = scaled_adult_nicos)) +
  geom_point(color= crab_eggs) +
  theme_gray() +
  geom_smooth(method = lm, level = 0.95, color = "black") +
  ylab("Aadult Nicothoids") +
  xlab("Proportion Viable Crab Eggs (per One Million Eggs)") + 
  #ggtitle("Egg Mortality Across Development Days") +
  theme(plot.title = element_text(hjust = 0.5)) 
