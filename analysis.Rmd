...
```{r}
low <- egg_mortality_day %>%
  filter(!crab_id %in% c("ANT-GAV-031", "ANT-GAV-032", "ANT-GAV-033", "ANT-GAV-035", "ANT-GAV-043", "ANT-GAV-034", "ANT-GAV-037", "ANT-GAV-038", "ANT-GAV-045"))

low_late <- egg_mortality_day %>%
  mutate(total_nicos = (number_adult_nicos + number_juvenile_nicos)) %>%
...
summary_low_late <- summary(low_late$prop_nonviable_eggs)
summary_low_late

med <- egg_mortality_day %>%
  filter(crab_id %in% c("ANT-GAV-031", "ANT-GAV-032", "ANT-GAV-033", "ANT-GAV-035"))

med_late <- egg_mortality_day %>%
  mutate(total_nicos = (number_adult_nicos + number_juvenile_nicos)) %>%
  mutate(density_adult_nicos = ((number_adult_nicos/total_number_crab_eggs)*1000)) %>%
  mutate(density_juvenile_nicos = ((number_juvenile_nicos/total_number_crab_eggs)*1000)) %>%
  mutate(density_total_nicos = ((total_nicos/total_number_crab_eggs)*1000)) %>%
...
#look at the summary stats for high infestation crabs
summary_med_late <- summary(med_late$prop_nonviable_eggs)
summary_med_late

high <- egg_mortality_day %>%
  filter(crab_id %in% c("ANT-GAV-034", "ANT-GAV-037", "ANT-GAV-038", "ANT-GAV-045"))

high_late <- egg_mortality_day %>%
  mutate(total_nicos = (number_adult_nicos + number_juvenile_nicos)) %>%
  mutate(density_adult_nicos = ((number_adult_nicos/total_number_crab_eggs)*1000)) %>%
  mutate(density_juvenile_nicos = ((number_juvenile_nicos/total_number_crab_eggs)*1000)) %>%
  mutate(density_total_nicos = ((total_nicos/total_number_crab_eggs)*1000)) %>%
...
#look at the summary stats for high infestation crabs
summary_high_late <- summary(high_late$prop_nonviable_eggs)
summary_high_late

#create a data frame with the average values across each day 
high_vis_data <- high_late %>%
  group_by(day) %>%
  summarize(avg_prop_viable_eggs = mean(prop_viable_eggs),
            avg_prop_nonviable_eggs = mean(prop_nonviable_eggs)) %>%
  as_tibble()
...
high_vis <- ggplot(data = high_vis_data, 
       aes(x = day, y = avg_prop_viable_eggs)) +
  geom_point(color = crab_eggs) +
  theme_gray() +
  ylab("Average Proportion Viable Crab Eggs") +
...
```{r}
#remove the most highly infested crabs (ANT-GAV-037, ANT-GAV-043, and ANT-GAV-045)
...
high_infested <- egg_mortality_day %>%
  filter(crab_id %in% c("ANT-GAV-037", "ANT-GAV-045"))
...
low_crab_id <- crab_id %>%
  filter(avg_total_nicos <= 20)


#med infestation crabs
med_crab_id <- crab_id %>%
  filter(between(avg_total_nicos, 20, 90)) 

#high infestation crabs
high_crab_id <- crab_id %>%
  filter(avg_total_nicos >= 90) 
...
egg_mortality_density_low <- egg_mortality_density %>%
  filter(!crab_id %in% c("ANT-GAV-037", "ANT-GAV-043", "ANT-GAV-045"))


#calculate the mean density and standard deviation of each of the values for the low infestation crabs (all the crabs expect ANT-GAV-037, ANT-GAV-043, and ANT-GAV-045)
...
#calculate the mean density and standard deviation of each of the values for the high infestation crabs (ANT-GAV-037, ANT-GAV-043, and ANT-GAV-045)
