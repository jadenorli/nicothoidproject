...
eggs_mort_all <- read_csv("NicothoidProjectAnalysis/RawData/eggmortality2.csv")%>% 
  ...
  mutate(log_normal_adult_nicos = dlnorm(number_adult_nicos)) %>%
  mutate(log_normal_juvenile_nicos = dlnorm(number_juvenile_nicos)) %>%
  mutate(log_normal_nico_eggs = dlnorm(number_nico_eggs))
...
eggs_mort_avg <- eggs_mort_all %>%
  ...
  summarize(avg_prop_viable_eggs = mean(prop_viable_eggs),
            ...
            avg_log_adult_nicos = mean(log_normal_adult_nicos),
            avg_log_juvenile_nicos = mean(log_normal_juvenile_nicos),
            avg_log_nico_eggs = mean(log_normal_nico_eggs)) %>%
  as_tibble()
...
autoinfection_all <- read_csv("NicothoidProjectAnalysis/RawData/autoinfection_all.csv") %>%
  ...
  mutate(log_normal_adult_copepods = dlnorm(number_adult_copepods)) %>%
  mutate(log_normal_copepod_eggs = dlnorm(number_copepod_eggs))
