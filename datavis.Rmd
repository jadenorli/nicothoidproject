...
eggs_mort_all <- read_csv("LifeCycleExport2.csv")%>% 
  clean_names() %>% 
  select(!16:27) %>% 
  mutate(prop_viable_eggs = number_viable_crab_eggs/total_number_crab_eggs) %>% 
...
autoinfection_norm <- predict(autoinfection_process, as_data_frame(autoinfection_avg)) %>%
  as_data_frame()
...
autoinfection_vis <- ggplot(data = auotinfection_long, 
  ...
  scale_color_manual(values = c("#FDD49E","#EF6548","#D7301F", "#7F0000"),
